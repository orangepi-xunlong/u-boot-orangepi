...
int NAND_VersionCheck(void)
{
    ...
    ********************************************************************************/
    ...
    printf("Current nand driver version is %x %x %x %x \n", nand_version[0], \
            nand_version[1], nand_version[2], nand_version[3]);
    ...
    for(block_index=uboot_start_block;block_index<uboot_next_block;block_index++)
    {
        ...
        nand_physic_read_page(boot0_readop->chip,boot0_readop->block,boot0_readop->page,\
...
int NAND_UbootProbe(void)
{
    ...
    /* Fixed me
     * V3S/MR100 has no Nand controller
     */
    debug("Have no Nand controller support !!\n");
    return -1;
