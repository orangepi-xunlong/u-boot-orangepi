...
static void LCD_panel_init3(u32 sel);
...
static s32 LCD_open_flow(u32 sel)
{
	...
	LCD_OPEN_FUNC(sel, LCD_panel_init2, 120);
	LCD_OPEN_FUNC(sel, LCD_panel_init3, 5);
...
static void LCD_panel_init3(u32 sel)
{
	...
	for (i = 0; ; i++) {
		if (lcm_initialization_setting1[i].count == REGFLAG_END_OF_TABLE)
			break;

#ifdef SUPPORT_DSI
		else
			dsi_dcs_wr(sel, lcm_initialization_setting1[i].cmd,
						lcm_initialization_setting1[i].para_list,
						lcm_initialization_setting1[i].count);
#endif
		//break;
	}
}
...
__lcd_panel_t CC08021801_310_800X1280_mipi_panel = {
