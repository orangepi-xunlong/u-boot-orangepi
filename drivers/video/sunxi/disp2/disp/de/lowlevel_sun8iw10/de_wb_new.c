...
#define wb_writel(val, addr) (*((volatile unsigned long  *)(addr)) = (unsigned long)(val))
#define wb_readl(addr) (*((volatile unsigned long  *)(addr)))
...
int WB_eink_Set_Reg_Base(unsigned int sel, unsigned int base)
{
	...
	//reg = wb_readl(base+0x0c);
	//reg &= 0xfffff0ff;
	//reg |= 0x00000400;
	//wb_writel(reg, base+0x0c);
	reg = wb_readl(base+0x0c);
	printk("%s: reg = 0x%x, set write back base = 0x%x\n", __func__, reg, (unsigned int)wb_dev[sel]);
	return 0;
}
...
{
  int reg;
  reg=wb_readl(wb_dev[sel]+0x0);
  reg|=1;
	wb_writel(reg, wb_dev[sel]+0x0);
...
int WB_eink_Set_Para(unsigned int sel, __einkwb_config_t *cfg)
{
	...
	wb_writel((0x10000000|(in_port<<16)|(self_sync == 1)?0x20:0x0), wb_dev[sel]+0x0);
	//__inf("gctrl_adrr=0x%x, val=0x%x\n", (unsigned int)(&(wb_dev[sel]->gctrl)), wb_dev[sel]->gctrl.dwval);
	//input size
	wb_writel(((in_w-1)|((in_h-1)<<16)), wb_dev[sel]+0x4);
	//wb_dev[sel]->size.dwval = (in_w-1)|((in_h-1)<<16);
	//input crop window
	
	wb_writel((crop_x|((crop_y)<<16)), wb_dev[sel]+0x8);
	//wb_dev[sel]->crop_coord.dwval = crop_x|((crop_y)<<16);
	wb_writel( ((crop_w - 1)|((crop_h - 1)<<16)), wb_dev[sel]+0xc);
	...
	//output fmt
	wb_writel( out_fmt, wb_dev[sel]+0x44);
	...
	if(out_fmt == 9/*DISP_FORMAT_Y*/)
	{
		...
		out_addr[1] = 0;
		out_addr[2] = 0;
		wb_writel(out_addr[0], wb_dev[sel]+0x20);
		//wb_dev[sel]->wb_addr_A0.dwval = out_addr[0];
		//wb_dev[sel]->wb_addr_A1.dwval = 0;
		//wb_dev[sel]->wb_addr_A2.dwval = 0;
		wb_writel(out_buf_w, wb_dev[sel]+0x30);
		//wb_dev[sel]->wb_pitch0.dwval = out_buf_w;
		//wb_dev[sel]->wb_pitch1.dwval = 0;

	  //CSC
		wb_writel(0x00000001, wb_dev[sel]+0x54);
		//wb_dev[sel]->bypass.dwval |= 0x00000001;
  	if(csc_std==0 || csc_std>2)//bt601_limit
  	{
    	wb_writel(rgb2yuv601[0][0], wb_dev[sel]+0x100);
    	wb_writel(rgb2yuv601[0][1], wb_dev[sel]+0x104);
    	wb_writel(rgb2yuv601[0][2], wb_dev[sel]+0x108);
    	wb_writel(rgb2yuv601[0][3], wb_dev[sel]+0x10c);
	  ...
	  {
    	wb_writel(rgb2yuv709[0][0], wb_dev[sel]+0x100);
    	wb_writel(rgb2yuv709[0][1], wb_dev[sel]+0x104);
    	wb_writel(rgb2yuv709[0][2], wb_dev[sel]+0x108);
    	wb_writel(rgb2yuv709[0][3], wb_dev[sel]+0x10c);
	  ...
	  {
    	wb_writel(r2gray[0][0], wb_dev[sel]+0x100);
    	wb_writel(r2gray[0][1], wb_dev[sel]+0x104);
    	wb_writel(r2gray[0][2], wb_dev[sel]+0x108);
    	wb_writel(r2gray[0][3], wb_dev[sel]+0x10c);
...
int WB_eink_Get_Status(unsigned int sel)
{
	...
	status=wb_readl(wb_dev[sel]+0x4c);
	printk("status = 0x%x\n", status);
...
int WB_eink_EnableINT(unsigned int sel)
{
  ...
  int reg;
  reg=wb_readl(wb_dev[sel]+0x48);
  reg|=1;
	wb_writel(reg, wb_dev[sel]+0x48);
...
{
  int reg;
  reg=wb_readl(wb_dev[sel]+0x4c);
  reg|=(WB_END_IE | WB_FINISH_IE | WB_FIFO_OVERFLOW_ERROR_IE | WB_TIMEOUT_ERROR_IE);
	wb_writel(reg, wb_dev[sel]+0x4c);
