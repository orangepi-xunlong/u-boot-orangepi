...
struct disp_capture *disp_get_capture(u32 screen_id)
{
	...
	if ((!disp_al_query_be_mod(screen_id)) ||
	    (!bsp_disp_feat_get_capture_support(screen_id))) {
		/* DE_WRN("be %d mod is not registered or not */
		/* support capture\n", screen_id); */
		return NULL;
	}
							       ...
							       *capture)
{
	...
	if ((!disp_al_query_be_mod(capture->channel_id)) ||
	    (!bsp_disp_feat_get_capture_support(capture->channel_id))) {
...
static s32 disp_capture_shadow_protect(struct disp_capture *capture,
				       bool protect)
{
...
static s32 disp_capture_screen(struct disp_capture *capture,
			       disp_capture_para *para)
{
	...
	if (bsp_disp_get_output_type(capture->channel_id) ==
...
static s32 disp_capture_init(struct disp_capture *capture)
{
	...
	if ((!disp_al_query_be_mod(capture->channel_id)) ||
	    (!bsp_disp_feat_get_capture_support(capture->channel_id))) {
		DE_WRN("be %d is not register or capture %d isnot support\n",
...
s32 disp_init_capture(struct __disp_bsp_init_para *para)
{
	...
	num_screens = bsp_disp_feat_get_num_screens();
	captures = kmalloc_array(num_screens,
				 sizeof(struct disp_capture),
				 GFP_KERNEL | __GFP_ZERO);
	if (captures == NULL) {
		DE_WRN("malloc memory fail!\n");
		return DIS_FAIL;
	}
	capture_private = kmalloc_array(num_screens,
					sizeof(struct
					       disp_capture_private_data),
					GFP_KERNEL | __GFP_ZERO);
	if (capture_private == NULL) {
		DE_WRN("malloc memory fail!\n");
		return DIS_FAIL;
	}
	...
	for (screen_id = 0; screen_id < num_screens; screen_id++) {
		...
		capture->capture_screen_get_buffer_id =
		    disp_capture_screen_get_buffer_id;
