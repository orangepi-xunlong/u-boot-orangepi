...
static volatile struct __de_dsi_dev_t *dsi_dev[1];
...
__s32 dsi_start(__u32 sel, enum __dsi_start_t func)
{
	...
	if (func == DSI_START_HSC) {
		dsi_dev[sel]->dsi_inst_func[DSI_INST_ID_LP11]
		    .bits.lane_cen = 0;
	}
	return 0;
}
	...
	*(p++) = dsi_ecc_pro(dsi_dev[sel]->dsi_cmd_tx[0].dwval);
	...
	if (para_num > 1) {
		...
		*(p++) = cmd;
		...
		crc = dsi_crc_pro((__u8 *) (dsi_dev[sel]->dsi_cmd_tx + 1),
				  para_num + 1);
		*(p++) = (crc >> 0) & 0xff;
		*(p++) = (crc >> 8) & 0xff;
...
__s32 dsi_dcs_rd_memory(__u32 sel, __u32 *p_data, __u32 length)
{
	...
	while (rx_cntr) {
		...
		if (start) {
			dsi_dcs_rd(sel, DSI_DCS_READ_MEMORY_START,
				   rx_bf, &rx_num);
			start = 0;
		} else {
			dsi_dcs_rd(sel, DSI_DCS_READ_MEMORY_CONTINUE,
				   rx_bf, &rx_num);
		}
...
static __s32 dsi_basic_cfg(__u32 sel, disp_panel_para *panel)
{
	if (panel->lcd_dsi_if == LCD_DSI_IF_COMMAND_MODE) {
		...
		/* put start_delay to tcon. set ready sync */
		/* early to dramfreq, so set start_delay 1 */
		start_delay = 1;
		...
		if (panel->lcd_dsi_if == LCD_DSI_IF_BURST_MODE) {
			...
			edge1 =
			    sync_point + (panel->lcd_x + panel->lcd_hbp + 20)
			    * dsi_pixel_bits[panel->lcd_dsi_format]
			    / (8 * panel->lcd_dsi_lane);
...
static __s32 dsi_packet_cfg(__u32 sel, disp_panel_para *panel)
{
	...
	if ((panel->lcd_dsi_if == LCD_DSI_IF_VIDEO_MODE) ||
		...
		__u32 dsi_hsa, dsi_hact, dsi_hbp;
		__u32 dsi_hfp, dsi_hblk, dsi_vblk, tmp;
		__u32 vc = 0;
		__u32 x = panel->lcd_x;
		__u32 y = panel->lcd_y;
		...
		if (panel->lcd_dsi_if == LCD_DSI_IF_BURST_MODE) {
			...
			dsi_hbp =
			    (hbp - hspw) * dsi_pixel_bits[format] / 8 - (4 + 2);
			dsi_hact = x * dsi_pixel_bits[format] / 8;
			dsi_hblk =
			    (ht - hspw) * dsi_pixel_bits[format] / 8 - (4 + 4 +
									2);
			dsi_hfp =
			    dsi_hblk - (4 + dsi_hact + 2) - (4 + dsi_hbp + 2);
...
__u8 dsi_ecc_pro(__u32 dsi_ph)
{
	...
	ph.bits.bit29 = ph.bits.bit10 ^ ph.bits.bit11
	    ^ ph.bits.bit12 ^ ph.bits.bit13
	    ^ ph.bits.bit14 ^ ph.bits.bit15
	    ^ ph.bits.bit16 ^ ph.bits.bit17
	    ^ ph.bits.bit18 ^ ph.bits.bit19
	    ^ ph.bits.bit21 ^ ph.bits.bit22 ^ ph.bits.bit23;
	ph.bits.bit28 = ph.bits.bit04 ^ ph.bits.bit05
	    ^ ph.bits.bit06 ^ ph.bits.bit07
	    ^ ph.bits.bit08 ^ ph.bits.bit09
	    ^ ph.bits.bit16 ^ ph.bits.bit17
	    ^ ph.bits.bit18 ^ ph.bits.bit19
	    ^ ph.bits.bit20 ^ ph.bits.bit22 ^ ph.bits.bit23;
	ph.bits.bit27 = ph.bits.bit01 ^ ph.bits.bit02
	    ^ ph.bits.bit03 ^ ph.bits.bit07
	    ^ ph.bits.bit08 ^ ph.bits.bit09
	    ^ ph.bits.bit13 ^ ph.bits.bit14
	    ^ ph.bits.bit15 ^ ph.bits.bit19
	    ^ ph.bits.bit20 ^ ph.bits.bit21 ^ ph.bits.bit23;
	ph.bits.bit26 = ph.bits.bit00 ^ ph.bits.bit02
	    ^ ph.bits.bit03 ^ ph.bits.bit05
	    ^ ph.bits.bit06 ^ ph.bits.bit09
	    ^ ph.bits.bit11 ^ ph.bits.bit12
	    ^ ph.bits.bit15 ^ ph.bits.bit18
	    ^ ph.bits.bit20 ^ ph.bits.bit21 ^ ph.bits.bit22;
	ph.bits.bit25 = ph.bits.bit00 ^ ph.bits.bit01
	    ^ ph.bits.bit03 ^ ph.bits.bit04
	    ^ ph.bits.bit06 ^ ph.bits.bit08
	    ^ ph.bits.bit10 ^ ph.bits.bit12
	    ^ ph.bits.bit14 ^ ph.bits.bit17
	    ^ ph.bits.bit20 ^ ph.bits.bit21 ^ ph.bits.bit22 ^ ph.bits.bit23;
	ph.bits.bit24 = ph.bits.bit00 ^ ph.bits.bit01
	    ^ ph.bits.bit02 ^ ph.bits.bit04
	    ^ ph.bits.bit05 ^ ph.bits.bit07
	    ^ ph.bits.bit10 ^ ph.bits.bit11
	    ^ ph.bits.bit13 ^ ph.bits.bit16
	    ^ ph.bits.bit20 ^ ph.bits.bit21 ^ ph.bits.bit22 ^ ph.bits.bit23;
	...
	if (pd_bytes > 0) {
		for (byte_cntr = 0; byte_cntr < pd_bytes; byte_cntr++) {
			...
			for (bit_cntr = 0; bit_cntr < 8; bit_cntr++) {
				if (((crc & 0x0001) ^ ((0x0001 * curr_data) &
						       0x0001)) > 0)
					crc = ((crc >> 1) & 0x7fff)
					    ^ gen_code;
				else
