...
static s32 disp_smbl_update_regs(struct disp_smbl *smbl)
{
...
#if defined(__LINUX_PLAT__)
	{
		...
		if ((!smbl->manager) || (!smbl->manager->is_enabled) ||
		    (!smbl->manager->is_enabled(smbl->manager)) ||
		    (!smblp->info_dirty)) {
#if defined(__LINUX_PLAT__)
...
#endif
	/* can't call update_regs at apply at */
	/* single register buffer plat(e.g., a23,a31) */
...
static s32 disp_smbl_sync(struct disp_smbl *smbl)
{
	...
	if (smblp->shadow_info_dirty || smblp->enabled) {
		...
		backlight_dimming =
		    disp_al_smbl_get_backlight_dimming(smbl->channel_id);
	...
	if (smblp->shadow_info_dirty && (smblp->enabled == 0)) {
		disp_notifier_call_chain(DISP_EVENT_BACKLIGHT_DIMMING_UPDATE,
					 smbl->channel_id, (void *)256);
	}
#if defined(__LINUX_PLAT__)
	{
		unsigned long flags;
				       ...
				       u32 event, u32 sel, void *data)
{
	...
	case DISP_EVENT_OUTPUT_ENABLE:
		...
		if (enable && (output_type == DISP_OUTPUT_TYPE_LCD))
...
s32 disp_init_smbl(struct __disp_bsp_init_para *para)
{
	...
	smbls = kmalloc_array(num_smbls,
			      sizeof(struct disp_smbl),
			      GFP_KERNEL | __GFP_ZERO);
	if (smbls == NULL) {
		DE_WRN("malloc memory fail!\n");
		return DIS_FAIL;
	}
	smbl_private = kmalloc_array(num_smbls,
				     sizeof(struct disp_smbl_private_data),
				     GFP_KERNEL | __GFP_ZERO);
