...
unsigned long ir_packet_handle(struct ir_raw_buffer *ir_raw)
{
	int i = 0, ret = 0;
        ...
        ret = script_parser_fetch("ir_boot_recovery", "ir_press_times", (int *)&ir_press_times, sizeof(int) / 4);
        if (ret)
        {
	...
	if (ir_detect_count == ir_press_times)
	{
		...
		ret=script_parser_fetch("ir_boot_recovery", "ir_work_mode", (int *)&ir_work_mode, sizeof(int) / 4);
		if(ret)
		{
