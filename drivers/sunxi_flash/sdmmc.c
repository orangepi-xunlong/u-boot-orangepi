...
{
	if (nblock == 0) {
		pr_msg("%s: @nr is 0, erase from @from to end\n", __FUNCTION__);
...
{
	if (nblock == 0) {
		pr_msg("%s: @nr is 0, wipe from @from to end\n", __FUNCTION__);
		nblock = mmc_sprite->block_dev.lba - start_block - 1;
	}
...
int sdmmc_init_for_boot(int workmode, int card_no)
{
	...
	mmc_no = card_no;
	if(!mmc_boot){
		pr_msg("fail to find one useful mmc card\n");
		return -1;
	}
...
int sdmmc_init_for_sprite(int workmode)
{
	...
	mmc_no = 2;
	if(!mmc_sprite){
		pr_msg("fail to find one useful mmc card2\n");
#ifdef CONFIG_MMC3_SUPPORT
                pr_msg("try to find card3 \n");
                board_mmc_pre_init(3);
                mmc_sprite = find_mmc_device(3);
		mmc_no = 3;
                if(!mmc_sprite)
                {
                        pr_msg("try card3 fail \n");
                        return -1;
                }
                else
                {
...
static int check_secure_storage_key(unsigned char *buffer)
{
	...
	if( obj->magic != STORE_OBJECT_MAGIC ){
		pr_msg("Input object magic fail [0x%x]\n", obj->magic);
		return -1 ;
	}

	if( obj->crc != crc32( 0 , (void *)obj, sizeof(*obj)-4 ) ){
		pr_msg("Input object crc fail [0x%x]\n", obj->crc);
		return -1 ;
	}
	return 0;
}
...
static int check_secure_storage_map(unsigned char *buffer)
{
	...
	{
		pr_error("Item0 (Map) magic is bad\n");
		return 2;
	}
	if (map_buf->crc != crc32( 0 , (void *)map_buf, sizeof(struct map_info)-4 ))
	{
		pr_error("Item0 (Map) crc is fail [0x%x]\n", map_buf->crc);
		return -1;
	}
	return 0;
}
...
static int mmc_secure_storage_read_key(int item, unsigned char *buf, unsigned int len)
{
	...
	if (!ret)
	{
		...
		if (!check_secure_storage_key(align)){
			pr_msg("the secure storage item%d copy0 is good\n",item);
			goto ok ; /*copy 0 pass*/
		}
	...
	if (!ret)
	{
		/*check copy 1 */
		if (!check_secure_storage_key(align)){
			pr_msg("the secure storage item%d copy1 is good\n",item);
			goto ok ; /*copy 1 pass*/
		}
...
static int mmc_secure_storage_read_map(int item, unsigned char *buf, unsigned int len)
{
	...
	if (!ret)
	{
		...
		if (ret == 0)
		{
			...
			pr_error("the secure storage item0 copy 0 crc fail, the data is bad\n");
	}
	...
	if (!ret)
	{
		ret = check_secure_storage_map(align);
		if (ret == 0){
			pr_msg("the secure storage item0 copy1 is good\n");
			goto ok ;
		}else if (ret == 2){
			if (have_map_copy0 && !memcmp(map_copy0_buf, align, len))
			{
				pr_msg("the secure storage item0 copy0 == copy1, the data is good\n");
				goto ok ; /*copy have no magic and crc*/
			}
			else
			{
				pr_error("the secure storage item0 copy0 != copy1, the data is bad\n");
				free(map_copy0_buf);
				return -1;
			}
		}else{
			pr_error("the secure storage item0 copy 1 crc fail, the data is bad\n");
			free(map_copy0_buf);
			return -1;
		}
	}
