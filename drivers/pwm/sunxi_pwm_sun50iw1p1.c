...
static int sunxi_pwm_pin_set_state(char *dev_name, char *name)
{
	...
	if (len > 32)
		pr_error("disp_sys_set_state, size of mian_name is out of range\n");

	ret = fdt_set_all_pin(compat, (1 == state)?"pinctrl-0":"pinctrl-1");
	if (0 != ret)
		pr_error("%s, fdt_set_all_pin, ret=%d\n", __func__, ret);
...
/*
	...
	if (ret < 0) {
		pr_error("failed to get reg_busy_offset! err=%d\n", ret);
		goto err;
	}

	ret = fdt_getprop_u32(working_fdt, node, "reg_busy_shift", &config->reg_busy_shift);
	if (ret < 0) {
		pr_error("failed to get reg_busy_shift! err=%d\n", ret);
		goto err;
	}
...
int pwm_request(int pwm, const char *label)
{
	...
	if (!pchip) {
		pr_error("%s: error:pwm chip malloc failed!\n", __func__);
		return -1;
	} else {
	...
	if (ret < 0) {
		pr_error("fdt_getprop_u32 %s.%s fail\n", main_name, sub_name);
		goto err_pwm;
	}
	...
	if (clk_count == 1) {
		...
		if (pchip->pwm_clk == NULL) {
			pr_error("%s: can't get pwm clk\n", __func__);
			goto err_pwm;
		}
		ret = clk_prepare_enable(pchip->pwm_clk);
		if (ret) {
			pr_error("failed to enable pwm clock\n");
			goto err_pwm;
		}
	}
#endif
	...
	if (sub_node < 0) {
		pr_error("error:fdt %s err returned %s\n", sub_name,
	...
	pr_msg("request pwm success, %s:pwm%d:0x%x.\n", sub_name, pwm,
	       pchip->base);
