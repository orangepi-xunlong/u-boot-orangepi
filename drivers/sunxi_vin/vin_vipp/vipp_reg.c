...
unsigned long vipp_base[MAX_VIPP_NUM];
...
void vipp_set_crop(unsigned int id, struct vipp_crop *crop)
{
	...
	VIPP_CROP_START_POSITION_REG_t vipp_crop_start;
	VIPP_CROP_SIZE_REG_t vipp_crop_size;
	vipp_crop_start.dwval = 0;
	vipp_crop_start.bits.crop_hor_st = crop->hor;
	vipp_crop_start.bits.crop_ver_st = crop->ver;
	vipp_crop_size.dwval = 0;
	vipp_crop_size.bits.crop_width = crop->width;
	vipp_crop_size.bits.crop_height = crop->height;
	vin_reg_writel(vipp_base[id] + VIPP_CROP_START_REG_OFF, vipp_crop_start.dwval);
	vin_reg_writel(vipp_base[id] + VIPP_CROP_SIZE_REG_OFF, vipp_crop_size.dwval);
#endif
}
