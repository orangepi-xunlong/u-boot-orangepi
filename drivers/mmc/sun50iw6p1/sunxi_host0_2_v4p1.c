				...
				struct mmc_data *data)
{
	...
	if (data) {
		...
		if (data->flags & MMC_DATA_WRITE)
			cmdval |= (1 << 10);
		if ((IS_SD(mmc) || mmc->cfg->platform_caps.set_block_count == 0) && data->blocks > 1&&!(cmd->flags&MMC_CMD_MANUAL))
			cmdval |= (1 << 12);
	...
	if (data) {
		...
		do {
			...
			if ((IS_SD(mmc) || mmc->cfg->platform_caps.set_block_count == 0) && (data->blocks > 1)&&!(cmd->flags&MMC_CMD_MANUAL))//not wait auto stop when MMC_CMD_MANUAL is set
			{
				if (usedma)
