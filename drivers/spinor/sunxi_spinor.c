...
{
	unsigned  int       start_block;// ==addrlo/128
	unsigned  int       index_cnt;// ==lenlo/128
};
...
static int __spinor_erase_all(void)
{
	...
	do
	{
		...
		__msdelay(1000);
		printf(".");
		#ifdef CONFIG_SUNXI_MODULE_SPRITE
		sprite_cartoon_upgrade(count*0x52000*60/(total_write_bytes)+10);
		#endif
	} while (status & 0x01);
	printf("\nstatus = %d\n",status);
	printf("count = %d \n",count);
...
int spinor_size(void)
{
    ...
    nodeoffset =  fdt_path_offset(working_fdt,"/soc/norflash");
    if(nodeoffset > 0)
    {
            ret = fdt_getprop_u32(working_fdt, nodeoffset,"size",(uint32_t*)&size);
    }
...
int spinor_datafinish_prot(void)
{
	uint  id = 0xff;
	int   ret;
	int i = 0;
	sunxi_mbr_t *  img_mbr;//sunxi_mbr_t
	...
	img_mbr = (sunxi_mbr_t *)malloc(1024 * 1024);
	img_mbr = (sunxi_mbr_t *)(spinor_store_buffer+CONFIG_SPINOR_LOGICAL_OFFSET*512);\
	if(!strncmp((const char*)img_mbr->magic, SUNXI_MBR_MAGIC, 8))
	{
		tick_printf("fetch mbr success!\n");
		//__dump_mbr(img_mbr);
	}else{
		printf("sunxi sprite error : fetch mbr error!\n");
	}

	part_info = img_mbr->array;
	for(part_info = img_mbr->array, i=0;i<img_mbr->PartCount;i++, part_info++)
	{
		if(!strcmp("private", (const char *)part_info->name) || 0x8000 == part_info->keydata)
		{
			protect_par[num_protect_par].start_block = (part_info->addrlo)/128;
			protect_par[num_protect_par].index_cnt = (part_info->lenlo)/128;
			printf("detected %s partition to be protected!\nstart_block = 0x%x,    index_cnt = 0x%x\n",part_info->name,protect_par[num_protect_par].start_block,protect_par[num_protect_par].index_cnt);
	...
	if(num_protect_par>0)
	{
		...
		{
			for(;break_num<num_protect_par;break_num++)// if @ the keydata protect partition, break to erase
			{
				if((i>=protect_par[break_num].start_block)&&(i<(protect_par[break_num].start_block+protect_par[break_num].index_cnt)))
				{
					debug("\nbreak protect partition: sector = %d, byte = 0x%x\n",i,i*NOR_ERASE_SECTOR);
					continue;
				}
				else
				{
			...
			__spinor_erase_block(i);//spinor_erase_one_block(i);
			printf(".");
			sprite_cartoon_upgrade((i*60)/(total_write_bytes/(NOR_ERASE_SECTOR))+10);
			if(!(i%128))
			{
				printf("\n");
			}
		...
		break_num = 0;
		for(;break_num<num_protect_par;break_num++)// if @ the keydata protect partition, reset write buffer
		{
			memset(spinor_store_buffer+(protect_par[break_num].start_block*NOR_ERASE_SECTOR),'\0',protect_par[break_num].index_cnt*NOR_ERASE_SECTOR);
			debug("\nbreak write partition: sector = %d, byte = 0x%x\nlen: sector = %d, byte = 0x%x\n",protect_par[break_num].start_block,protect_par[break_num].start_block*NOR_ERASE_SECTOR,protect_par[break_num].index_cnt,protect_par[break_num].index_cnt*NOR_ERASE_SECTOR);
		}
	}
	else
	{
