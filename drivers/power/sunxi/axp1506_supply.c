...
static int axp1506_set_vol(char* name, int set_vol, int onoff)
{
	u8   reg_value;
	...
	if(axp_i2c_read(AXP1506_ADDR, p_item->cfg_reg_addr, &reg_value))
    {
        return -1;
    }
	...
	if(axp_i2c_write(AXP1506_ADDR, p_item->cfg_reg_addr, reg_value))
	{
	...
	if(axp_i2c_read(AXP1506_ADDR, p_item->ctrl_reg_addr, &reg_value))
	{
		return -1;
	}
	if(onoff == 0)
	...
	if(axp_i2c_write(AXP1506_ADDR, p_item->ctrl_reg_addr, reg_value))
	{
...
static int axp1506_probe_vol(char *name)
{
	u8  reg_value;
	axp_contrl_info* p_item = NULL;
	u8 base_step;
...
int axp1506_set_supply_status(int vol_name, int vol_value, int onoff)
{
	return -1;
}

int axp1506_set_supply_status_byname(char *vol_name, int vol_value, int onoff)
{

	return axp1506_set_vol(vol_name, vol_value, onoff);
}

int axp1506_probe_supply_status(int vol_name, int vol_value, int onoff)
{
	return -1;
}

int axp1506_probe_supply_status_byname(char *vol_name)
{
	return axp1506_probe_vol(vol_name);
}
