...
{
	int node = -1;
	unsigned int boot_disp, boot_disp1, boot_disp2;
	
	node = fdt_path_offset(working_fdt, "disp");
	if (node >= 0) {
		fdt_getprop_u32(working_fdt, node, "boot_disp", (uint32_t*)&boot_disp);
		fdt_getprop_u32(working_fdt, node, "boot_disp1", (uint32_t*)&boot_disp1);
		fdt_getprop_u32(working_fdt, node, "boot_disp2", (uint32_t*)&boot_disp2);
	}
	else{
		printf("disp node not found!\n");
		return -1;
	}
	
	setenv_ulong("boot_disp", boot_disp);
	setenv_ulong("boot_disp1", boot_disp1);
	setenv_ulong("boot_disp2", boot_disp2);
...
int fb_update_cmdline(unsigned int fb_id)
{
        ...
        if (size && fb->cv->base) {
                ...
                ret = setenv("disp_reserve", disp_reserve);
                if (ret)
