...
CONFIG_BIN_PATH=bin/axp802_bin
else
CONFIG_BIN_PATH=bin
endif
...
u-boot-$(CONFIG_TARGET_NAME).bin:   u-boot.bin
	@cp -v $<    $@
	@cp -v $@ $(objtree)/../../tools/pack/chips/$(CONFIG_SYS_BOARD)/$(CONFIG_BIN_PATH)/$@
	if [ -z "$(findstring $(OTA_TEST_NAME), $(SUNXI_MODE))" ]; then \
		cp -v u-boot-$(CONFIG_TARGET_NAME).bin $(objtree)/../../tools/pack/chips/$(subst  spinor-,,$(CONFIG_TARGET_NAME))/$(CONFIG_BIN_PATH)/u-boot-$(CONFIG_TARGET_NAME).bin; \
	else \
		cp -v u-boot-$(CONFIG_TARGET_NAME).bin $(objtree)/../../tools/pack/chips/$(subst  spinor-,,$(CONFIG_TARGET_NAME))/$(CONFIG_BIN_PATH)/u-boot-$(CONFIG_TARGET_NAME)-$(SUNXI_MODE).bin; \
	fi
