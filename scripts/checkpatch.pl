...
# clk|pinctrl|uart|dma|rtc|gic|timer|mbus|sid|sysinfo|iommu|pmu|cpufreq|devfreq|thermal|standby|hotplug|cpuidle|pd|rpm|disp|drm|g2d|di|usb|twi|spi|ir|gpadc|lradc|ledc|can|pwm|emac|wifi|bt|mali-utgard|mali-midgard|mali-bifrost|img-sgx|img-rgx
		if ($in_commit_log && $line =~ /^\ {4}K[01]:sun(?:xi|[1-9][0-9]?i(w[1-9][0-9]?)?(p[1-9])?):P[012]:(?:script|clk|pinctrl|uart|dma|rtc|gic|timer|mbus|sid|sysinfo|iommu|pmu|cpufreq|devfreq|thermal|standby|hotplug|cpuidle|pd|rpm|disp|drm|g2d|di|usb|twi|spi|ir|gpadc|lradc|ledc|can|pwm|emac|wifi|bt|mali-utgard|mali-midgard|mali-bifrost|img-sgx|img-rgx|nand|mmc|nor|mtd|ce|sid|camera|vin|io|tpadc|crypto|input|tee|ramdisk|watchdog|defconfig|min-system|ve|audio|dump_reg|tvd|dsp|tv|msgbox|gpio|plic|reboot|esl|cache|ddr|sprite|utility|build)/){
		    if ($line =~ /^\ {4}K0:.*/i){
			$sunxi_loglevel = 0;
		    }
...
# Check for Start kernel time for brandy
		if ($in_commit_log && $line =~ /^\ {4}Start kernel\s*:\s*(?:([0-9a-zA-Z\-]*\s*:\s*(?:secure|non-secure)\s*:\s*\d+\.\d+s)|NULL)/){
		    $sunxi_has_stime = 1;
		}
...
# check for sysctl duplicate constants
	...
	if ($has_commit_log && $sunxi_author_isaw && !$sunxi_isrevert && !$sunxi_has_loglevel){
	    ERROR("NO_COMMIT_LEVEL",
		    "Do not conform to the submission specification. Eg:\"K1:sun8i:P0:\[script|clk|pinctrl|uart|dma|rtc|gic|timer|mbus|sid|sysinfo|iommu|pmu|cpufreq|devfreq|thermal|standby|hotplug|cpuidle|pd|rpm|disp|drm|g2d|di|usb|twi|spi|ir|gpadc|lradc|ledc|can|pwm|emac|wifi|bt|mali-utgard|mali-midgard|mali-bifrost|img-sgx|img-rgx|nand|mmc|nor|mtd|ce|sid|carema|vin|io|tpadc|crypto|input|tee|ramdisk|watchdog|defconfig|min-system|ve|audio|dump_reg|tvd|dsp|tv|msgbox|gpio|plic|reboot|esl|cache|ddr|sprite|utility|build\]\n");
	}

	if ($has_commit_log && $sunxi_author_isaw && !$sunxi_isrevert && !$sunxi_has_pmsid){
	    ERROR("NO_PMS_ID",
		    "Without PMS TaskID,or PMS BugID. You can get the ID from PMS. Eg: PMS BugID: 42442\n");
	}
...
# Check for Start kernel time for brandy
	if ($has_commit_log && $sunxi_author_isaw && !$sunxi_isrevert && !$sunxi_has_stime){
	    ERROR("NO_Start_Time",
		    "require:Start kernel:[ic-board]:[secure|non-secure]:[time]s (or NULL)\n
		    eg: Start kernel:A100-b3:secure: 05.242s \n
		    Start kernel:NULL \n
		    where time come from uboot print \"\[05.242\]Starting kernel ...\"\n");
	}
