...
static int auto_update_fetch_mbr(void  *img_mbr)
{
	...
	if (!Img_Fat_ReadItem(imghd, imgitemhd, imgname, img_mbr,sizeof(sunxi_mbr_t) * SUNXI_MBR_COPY_NUM))
	{
...
static int auto_update_deal_fullimg(void)
{
	int  ret  = -1;
	int  ret1;
	uchar  *down_buff = NULL;
	...
	if(!down_buff)
	{
	    ...
	    goto __auto_update_deal_fullimg_err1;
	}
	...
	if(ret1 != 0)
	{
	    ...
	    goto __auto_update_deal_fullimg_err2;
	}
...
__auto_update_deal_fullimg_err1:
	sunxi_sprite_exit(1);
__auto_update_deal_fullimg_err2:
	if(down_buff)
	{
	    free(down_buff);
	}
