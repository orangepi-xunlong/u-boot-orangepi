...
void sunxi_ss_open(void)
{
	...
	reg_val = readl(CCMU_SS_SCLK_CTRL); //SS CLOCK
	reg_val &= ~(0xf<<24);
	reg_val |= 0x1<<24;
	reg_val &= ~(0x3<<16);
	reg_val |= 0x0<<16;
					...
					u8 *src_addr, u32 src_len)
{
	...
	total_len = __sha256_padding(src_len, (u8 *)src_addr)/4; //calculate plain text length
	...
	for(i=0; i< md_size; i++)
	{
	    dst_addr[i] = p_sign[i]; //read the abstract message info from dst addr
	}
	//clear pending
