...
{
#if 1	/* T7	:	clock = (24000000 * factor_n * factor_k) / 2;*/
	__u32 reg_val;
	__u32 factor_n, factor_k;
	__u32 clock;
...
#else  /*H6/A63 :	clock = 24000000 * factor_n / factor_m0/factor_m1/4;*/
	__u32 reg_val;
	__u32 factor_n;
	__u32 factor_m0;
	__u32 factor_m1;
...
#endif
	printf("CCMU_PLL_REG = 0x%x, clock = %d Hz\n",
		reg_val, clock);
	return clock;
}
...
int _change_ndfc_clk_v1(__u32 nand_index, __u32 dclk_src_sel, __u32 dclk, __u32 cclk_src_sel, __u32 cclk)
{
	...
	printf("CCM_NAND0_CLK0_REG = 0x%x\n", reg_val);
	printf("CCM_NAND0_CLK1_REG = 0x%x\n", reg_val);
	if (nand_index == 0) {
