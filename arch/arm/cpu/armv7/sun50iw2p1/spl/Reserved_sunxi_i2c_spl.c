...
int probe_axp_exist(void)
{
	u32 tmp = 0;
	u32 value = 0;

	//gpio mutex input
	tmp = readl(SUNXI_RPIO_BASE + 0x00);
	tmp &= (~(0xF << 0));
	writel(tmp, SUNXI_RPIO_BASE + 0x00);

	__usdelay(10);
	//pull down
	tmp = readl(SUNXI_RPIO_BASE + 0x1c);
	tmp &= (~(0x3 << 0));
	tmp |= (0x2 << 0);
	writel(tmp, SUNXI_RPIO_BASE + 0x1c);

	__usdelay(15);
	//Data
	value = readl(SUNXI_RPIO_BASE + 0x10);
	__usdelay(15);
