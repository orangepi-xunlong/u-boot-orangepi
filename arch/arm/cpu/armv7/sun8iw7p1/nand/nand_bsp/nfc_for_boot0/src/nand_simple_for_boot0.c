...
****************************************************************************/
void _add_cmd_list(NFC_CMD_LIST *cmd, __u32 value, __u32 addr_cycle,
					__u8 *addr, __u8 data_fetch_flag, __u8 main_data_fetch,
					__u32 bytecnt, __u8 wait_rb_flag)
{
	cmd->addr = addr;
...
*****************************************************************************/
void _cal_addr_in_chip(__u32 block, __u32 page,
				__u32 sector, __u8 *addr, __u8 cycle)
{
	__u32 row;
	__u32 column;
...
__s32 _read_status(__u32 cmd_value, __u32 nBank)
{
	...
	addr_cycle = 0;
	_add_cmd_list(&cmd_list, cmd_value,
		addr_cycle, addr, 1, NFC_IGNORE, 1, NFC_IGNORE);
...
__s32 _read_single_page(struct boot_physical_param *readop, __u8 read_mode)
{
	...
	_add_cmd_list(cmd_list + 1, 0x05, NFC_IGNORE, NFC_IGNORE,
		NFC_IGNORE, NFC_IGNORE, NFC_IGNORE, NFC_IGNORE);
	_add_cmd_list(cmd_list + 2, 0xe0, NFC_IGNORE, NFC_IGNORE,
		NFC_IGNORE, NFC_IGNORE, NFC_IGNORE, NFC_IGNORE);
	_add_cmd_list(cmd_list + 3, 0x30, NFC_IGNORE, NFC_IGNORE,
		NFC_IGNORE, NFC_IGNORE, NFC_IGNORE, NFC_IGNORE);
	list_len = 4;
	...
	if (SUPPORT_READ_RETRY && read_mode == 0) {
		/*toshiba or Samsung mode or micron*/
		...
		for (k = 0; k < READ_RETRY_CYCLE+1; k++) {
			...
			if (k > 0) {
				if (NFC_ReadRetry(readop->chip, RetryCount[readop->chip],
					READ_RETRY_TYPE)) {
			...
			NFC_RandomEnable();
			ret = NFC_Read(cmd_list, readop->mainbuf,
				sparebuf, read_mode , NFC_PAGE_MODE);
			NFC_RandomDisable();
...
__s32 PHY_GetDefaultParam(__u32 bank)
{
	...
	if (READ_RETRY_MODE == 2 || READ_RETRY_MODE == 3) {
		for (i = 8; i < 12; i++) {
			...
			if (ret >= 0 && oob[0] == 0x00 && oob[1] == 0x4F &&
...
__s32  PHY_ReadNandId(__s32 nChip, void *pChipID)
{
	...
	_add_cmd_list(&cmd, 0x90, 1 , &addr,
		NFC_DATA_FETCH, NFC_IGNORE, 6, NFC_NO_WAIT_RB);
...
__s32  PHY_GetFeature(__s32 nChip, __u8 *addr, void *value)
{
	...
	_add_cmd_list(&cmd, 0xEE, 1 , addr,
		NFC_DATA_FETCH, NFC_IGNORE, 4, NFC_NO_WAIT_RB);
...
__s32  PHY_SetFeature(__s32 nChip, __u8 *addr, void *value)
{
	...
	_add_cmd_list(&cmd, 0xEF, 1 , addr,
		NFC_DATA_FETCH, NFC_IGNORE, 4, NFC_WAIT_RB);
