...
void *malloc_noncache(uint num_bytes)
{
    ...
    {
	    if (ptr->next->address >=
		(ptr->address + ptr->size + 2 * sizeof(struct alloc_struct_t) +
		 actual_bytes)) {
		    break;
	    }
...
void  free_noncache(void *p)
{
    ...
    ptr =
	&boot_heap_head; /* look for the node which po__s32 this memory block */
    while (ptr && ptr->next) {
    ...
    prev->next = ptr->next; /* delete the node which need be released from the
			       memory block chain  */
