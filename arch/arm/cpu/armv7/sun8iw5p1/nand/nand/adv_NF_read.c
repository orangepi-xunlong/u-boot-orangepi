...
__s32 load_and_check_in_one_blk( __u32 blk_num, void *buf, __u32 size, __u32 blk_size)
{
    ...
    {
        status = NF_read( copy_base >> NF_SCT_SZ_WIDTH, (void *)buf, size >> NF_SCT_SZ_WIDTH ); // read in a backup
        if( status == NF_OVERTIME_ERR )
            return ADV_NF_OVERTIME_ERR;
        else if( status == NF_ECC_ERR )
        	continue;

	/*check the backup checksun OK,*/
	...
	return ADV_NF_ERROR;                              // not get good backup in current block
}
						 ...
						 __u32 size, __u32 blk_size, __u32 *blks )
{
         ...
         blk_num++ )
    {
        ...
        if( NF_read_status( blk_num ) == NF_BAD_BLOCK ) //if the current block is bad block,continue next block checking.
            continue;

        cur_blk_base = blk_num * blk_size;
        rest_size = size - buf_off ; //get  the rest size of the data
        size_loaded = ( rest_size < blk_size_load ) ?  rest_size : blk_size_load ;  //get loaded data size  in this time .
        if( NF_read( cur_blk_base >> NF_SCT_SZ_WIDTH, (void *)buf_base, size_loaded >> NF_SCT_SZ_WIDTH ) == NF_OVERTIME_ERR )
            return ADV_NF_OVERTIME_ERR;
    ...
    *blks = blk_num - start_blk;
	...
	else
	{
		...
		return ADV_NF_LACK_BLKS;  //block number is not enough
	}
}
