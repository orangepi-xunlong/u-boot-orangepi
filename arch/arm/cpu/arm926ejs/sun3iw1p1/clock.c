...
int sunxi_clock_get_ahb(void)
{
    ...
    switch(src)
    {
        ...
        case 1://src is OSC24M
            ...
            break;
        case 2://src is axi
            factor_div = (reg_val >> 4) & 0x03;
            clock = sunxi_clock_get_axi() >> factor_div;
            break;
        case 3://src is pll6(1x)/AHB1_PRE_DIV
            factor_pre_div = (reg_val >> 6) & 0x03;
            factor_div = (reg_val >> 4) & 0x03;
            clock = sunxi_clock_get_pll6() / (factor_pre_div + 1) / (factor_div + 1);
        break;
    }
...
void ccm_clock_enable(u32 clk_id)
{
    switch(clk_id>>8) {
        case AHB1_BUS0:
            ...
            ccm_debug("read s CCMU_BUS_CLK_GATING_REG0[0x%x]\n",readl(CCMU_BUS_CLK_GATING_REG0));
            break;
    }
}
...
void ccm_clock_disable(u32 clk_id)
{
    switch(clk_id>>8) {
        case AHB1_BUS0:
            ...
            ccm_debug("read dis CCMU_BUS_CLK_GATING_REG0[0x%x]\n",readl(CCMU_BUS_CLK_GATING_REG0));
            break;
    }
}
