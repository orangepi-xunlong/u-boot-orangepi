...
#define SUNXI_CLUSTER_PWROFF_GATING(cluster) \
	(SUNXI_RCPUCFG_BASE + 0x44 + (cluster<<6))
#define SUNXI_CPU_PWR_CLAMP(cluster, cpu)         \
	(SUNXI_RCPUCFG_BASE + 0x50 + (cluster<<6) + (cpu<<2))
#define SUNXI_CLUSTER_PWRON_RESET(cluster)       \
	(SUNXI_RCPUCFG_BASE + 0x40  + (cluster<<6))
#define SUNXI_CPU_SUBSYS_RESET                      \
	(SUNXI_RCPUCFG_BASE + 0xA0)

#define SUNXI_CLUSTER0_RST_CTRL     (SUNXI_C0_CPUX_CFG_BASE + 0x00)
#define SUNXI_CLUSTER1_RST_CTRL     (SUNXI_C1_CPUX_CFG_BASE + 0x00)
#define SUNXI_CLUSTER0_CTRL         (SUNXI_C0_CPUX_CFG_BASE + 0x10)
#define SUNXI_CLUSTER1_CTRL         (SUNXI_C1_CPUX_CFG_BASE + 0x10)
#define SUNXI_CLUSTER0_CPU_STATUS   (SUNXI_C0_CPUX_CFG_BASE + 0x80)
#define SUNXI_CLUSTER1_CPU_STATUS   (SUNXI_C1_CPUX_CFG_BASE + 0x80)
#define SUNXI_CLUSTER0_DBG_REG0     (SUNXI_C0_CPUX_CFG_BASE + 0xc0)
#define SUNXI_CLUSTER1_DBG_REG0     (SUNXI_C1_CPUX_CFG_BASE + 0xc0)

#define SUNXI_CLUSTER_CTRL(cluster)             \
	(cluster == 0 ? SUNXI_CLUSTER0_CTRL : SUNXI_CLUSTER1_CTRL)
#define SUNXI_CLUSTER_CPU_STATUS(cluster)        \
	(cluster == 0 ? SUNXI_CLUSTER0_CPU_STATUS : SUNXI_CLUSTER1_CPU_STATUS)
#define SUNXI_CLUSTER_RST_CTRL(cluster)              \
	(cluster == 0 ? SUNXI_CLUSTER0_RST_CTRL : SUNXI_CLUSTER1_RST_CTRL)
#define SUNXI_CLUSTER_DBG_REG0(cluster)                           \
	(cluster == 0 ? SUNXI_CLUSTER0_RST_CTRL : SUNXI_CLUSTER1_RST_CTRL)
...
{
	return readl(SUNXI_CLUSTER0_CPU_STATUS) & (1<<(16 + cpu));
}
...
static inline void sunxi_enable_cpu(int cpu)
{
	...
	value = readl(SUNXI_CLUSTER_RST_CTRL(0));
	value &= (~(0x1 << cpu));
	writel(value, SUNXI_CLUSTER_RST_CTRL(0));
	...
	value = readl(SUNXI_CLUSTER_DBG_REG0(0));
	value |= (0x1 << cpu);
	writel(value, SUNXI_CLUSTER_DBG_REG0(0));
...
static inline void sunxi_disable_cpu(int cpu)
{
	...
	value = readl(SUNXI_CLUSTER0_DBG_REG0);
	value &= (~(0x1 << cpu));
	writel(value, SUNXI_CLUSTER0_DBG_REG0);
    __usdelay(10);
