...
typedef struct
{
    ...
    unsigned char             zorder;     //layer priority,can get layer prio,but never set layer prio
...
typedef enum
{
    FB_MODE_SCREEN0 = 0,
    FB_MODE_SCREEN1 = 1,
    FB_MODE_DUAL_SAME_SCREEN_TB = 2,//2 screen, top:screen0, bottom:screen1
    FB_MODE_DUAL_DIFF_SCREEN_SAME_CONTENTS = 3,//2 screen have same contents;
}disp_fb_mode;
...
typedef enum
{
    ...
    DISP_INIT_MODE_TWO_DIFF_SCREEN_SAME_CONTENTS = 4,//fb0 for two different screen
...
{
    bool                  b_init;
    disp_init_mode        disp_mode;//0:single screen0(fb0); 1:single screen1(fb0);  
                                    //2:dual diff screen(fb0, fb1); 3:dual same screen
                                    //(fb0 up and down);4:dual diff screen same contents(fb0)
