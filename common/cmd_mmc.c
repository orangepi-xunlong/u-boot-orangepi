...
int do_card2_probe(cmd_tbl_t *cmdtp, int flag,
			 int argc, char * const argv[])
{
	struct mmc *mmc_boot = NULL;
	static int card2_init = 0;

	if(card2_init)
	{
		printf("card2 has inited\n");
		return 0;
	}

	board_mmc_pre_init(2);
	mmc_boot = find_mmc_device(2);
	if(!mmc_boot){
		printf("fail to find card2\n");
		return -1;
	}
	if (mmc_init(mmc_boot)) {
		puts("card2 init failed\n");
		return  -1;
	}
	card2_init = 1;
	return 0;
...
U_BOOT_CMD(
	sunxi_card2_probe, 1, 0, do_card2_probe,
	"probe sunxi card2 device",
	"sunxi_card2_probe"
);
