...
//convert bin-type key to string,so that dragonkey can display it
static void sunxi_bin_to_string(void *input_buf,int input_len, void *output_buf)
{
	int i;
	char *p_out = output_buf ,*p_in = input_buf;
	memset(output_buf,0x00,input_len*2);
...
int __sunxi_read_key_by_name(void *buffer, uint buff_len, int *read_len)
{
	...
	*read_len = sizeof(sunxi_usb_burn_key_info_t);
	...
	if(!key_info->type)
	{
		...
		sunxi_bin_to_string((void *)data_buff,key_data_len,(void *)(key_info->key_data));
