...
extern int check_mac_from_efuse(char *mac_name);
...
extern int sunxi_mac_char_to_int(char *mac_char, char *result);
int sunxi_mac2efuse_translation(sunxi_efuse_key_info_t *efuse_key_info)
{
	int ret = 0;
	char result[7] = {0};

	ret = sunxi_mac_char_to_int((char *)(efuse_key_info->key_data), result);
	if (ret) {
		printf("%s,%d, err\n", __func__, __LINE__);
		return -1;
	}

	memset(efuse_key_info->key_data, 0, efuse_key_info->len);
	efuse_key_info->len = 7;
	memcpy(efuse_key_info->key_data, result, 7);

	printf("translate success\n");
	return 0;
}
extern int sunxi_mac_int_to_char(char *mac_int, char* result);
int sunxi_efuse2mac_translation(char *efuse_data, int length)
{
	int ret = 0;
	char result[18] = {0};
	ret = sunxi_mac_int_to_char(efuse_data, result);
	if (ret) {
		printf("%s,%d,failed\n", __func__, __LINE__);
		return -1;
	}
	memcpy(efuse_data, result, 18);
...
int __sunxi_burn_key(u8 *buff, uint buff_len)
{
	...
	for(;key_count>0;key_count--, key_list++)
	{
		...
		if(!key_list->type)
		{
			...
			{
				if(!strcmp(EFUSE_ETHMAC_NAME, key_list->name) || !strcmp(EFUSE_WIFIMAC_NAME, key_list->name)) {
					if (key_list->len != 17) {
						printf("%s lenth error!!\n", key_list->name);
						return -1;
					}

					if (check_mac_from_efuse(key_list->name) > 0) {
						printf("nv2 has data,abort to burn\n");
						return -1;
					}

					if (sunxi_mac2efuse_translation(&efuse_key_info) < 0) {
						printf("mac translate fail!!\n");
						return -1;
					}
				}
...
int __sunxi_read_key_by_name(void *buffer, uint buff_len, int *read_len)
{
	...
	*read_len = sizeof(sunxi_usb_burn_key_info_t);
	...
	if(!key_info->type)
	{
		...
		if(!strcmp(EFUSE_ETHMAC_NAME, key_info->name) || !strcmp(EFUSE_WIFIMAC_NAME, key_info->name)) {
			printf("need to mac translation\n");
			if (sunxi_efuse2mac_translation(data_buff, key_data_len) < 0) {
				printf("mac translate fail!!\n");
				return -1;
			}
		}
